...
const ProjectCard = ({ title, description, image, link, className = '' }) => {
    return (
        <motion.div
            whileHover={{ y: -10 }}
            className={`bg-ivory-light rounded-xl shadow-lg overflow-hidden border border-champagne/40 ${className}`}
        >
            <div className="relative h-64 overflow-hidden">
                ...
                />
                <div className="absolute inset-0 bg-charcoal/15 hover:bg-transparent transition-colors duration-300"></div>
            </div>

            <div className="p-8">
                <h3 className="text-2xl font-bold text-charcoal mb-3">{title}</h3>
                <p className="text-taupe mb-6 line-clamp-3">{description}</p>
